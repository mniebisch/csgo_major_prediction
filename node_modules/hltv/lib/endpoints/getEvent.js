...
var getEvent = function (config) { return function (_a) {
    ...
    return __awaiter(_this, void 0, void 0, function () {
        var $, name, dateStart, dateEnd, prizePool, location, teams, prizeDistribution, formats, relatedEvents, mapPool;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    location = { name: $('img.flag').attr('title'), code: parsing_1.popSlashSource($('img.flag')).split('.')[0] };
                    ...
                    prizeDistribution = mappers_1.toArray($('.placement')).map(function (placementEl) { return ({
                        ...
                        team: placementEl.find('.team').children().length !== 0
                            ? { name: placementEl.find('.team a').text(), id: Number(placementEl.find('.team a').attr('href').split('/')[2]) }
                            : undefined
                    }); });
                    formats = mappers_1.toArray($('.formats tr')).map(function (formatEl) { return ({
                        type: formatEl.find('.format-header').text(),
                        description: formatEl.find('.format-data').text().split('\n').join(' ').trim()
                    }); });
                    ...
                    mapPool = mappers_1.toArray($('.map-pool-map-holder')).map(function (mapEl) { return mappers_1.getMapSlug(mapEl.find('.map-pool-map-name').text()); });
                    return [2, { name: name, dateStart: dateStart, dateEnd: dateEnd, prizePool: prizePool, teams: teams, location: location, prizeDistribution: prizeDistribution, formats: formats, relatedEvents: relatedEvents, mapPool: mapPool }];
            }
        });
    });
}; };
