        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    ...
                    rank = Number(t$('.profile-team-stat .right').first().text().replace('#', '')) || undefined;
                    arePlayerPicturesOfficial = mappers_1.toArray(t$('.overlayImageFrame')).length > 0;
                    playerSelector = arePlayerPicturesOfficial ? '.overlayImageFrame' : '.overlayImageFrame-square';
                    playerImageSelector = arePlayerPicturesOfficial ? '.bodyshot-team-img' : '.profileImage';
                    getPlayerId = function (el) { return arePlayerPicturesOfficial
                        ? Number(E.popSlashSource(el).split('.')[0])
                        : Number(el.attr('src').split('/').slice(-2, -1)); };
                    players = mappers_1.toArray(t$(playerSelector)).filter(E.hasChild('.playerFlagName .text-ellipsis')).map(function (playerEl) { return ({
